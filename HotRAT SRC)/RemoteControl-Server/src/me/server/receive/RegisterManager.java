...
public class RegisterManager {
    ...
    public RegisterManager(Socket socket,String IP) throws IOException {
        ...
        button2.addActionListener(a->{
            String[] modetypes = {"创建值","创建键"};
            String modetype = (String) JOptionPane.showInputDialog(null,"创建","类型",JOptionPane.QUESTION_MESSAGE,null,modetypes,modetypes[0]);
            switch (modetype) {
                case "创建值":
                    String name = JOptionPane.showInputDialog(null,"输入新值名","创建",JOptionPane.QUESTION_MESSAGE);
                    String typename = null;
                    String[] types = {"字符串型","长字符串型","长整型"};
                    String type = (String) JOptionPane.showInputDialog(null,"类型选择","类型",JOptionPane.QUESTION_MESSAGE,null,types,types[0]);
                    switch (type) {
                        case "字符串型":
                            typename = "REG_SZ";
                            break;
